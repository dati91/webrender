...
impl YamlFrameReader {
    ...
    pub fn new_from_args(s: &str) -> YamlFrameReader {
        let yaml_file = PathBuf::from(s);
    ...
    pub fn build(&mut self, api: &RenderApi, builder: &mut DisplayListBuilder, pipeline_id: PipelineId) {
        ...
        if let Some(pipelines) = yaml["pipelines"].as_vec() {
            for pipeline in pipelines {
                self.build_pipeline(api, pipeline["id"].as_pipeline_id().unwrap(), pipeline, builder);
            }
        }

        assert!(!yaml["root"].is_badvalue(), "Missing root stacking context");
        self.build_pipeline(api, pipeline_id, &yaml["root"], builder);
    }
    ...
    pub fn build_pipeline(
        ...
        let content_size = self.get_root_size_from_yaml(api, yaml);
        //let mut builder = DisplayListBuilder::new(pipeline_id, content_size);
        let mut info = LayoutPrimitiveInfo::new(LayoutRect::zero());
        self.add_stacking_context_from_yaml(builder, api, yaml, true, &mut info);
    ...
    fn to_image_mask(&mut self, item: &Yaml, api: &RenderApi) -> Option<ImageMask> {
        ...
        let file = match item["image"].as_str() {
            ...
            None => {
                println!("No image provided for the image-mask!");
                return None;
            }
        };

        let (image_key, image_dims) =
            self.add_or_get_image(&file, None, api);
                    ...
                    let source = match border_type {
                        "image" => {
                            ...
                            let (image_key, _) = self
                                .add_or_get_image(&file, None, api);
    ...
    fn handle_yuv_image(
        ...
        let yuv_data = match item["format"].as_str().expect("no format supplied") {
            "planar" => {
                ...
                let (y_key, _) = self.add_or_get_image(&y_path, None, api);

                let u_path = rsrc_path(&item["src-u"], &self.aux_dir);
                let (u_key, _) = self.add_or_get_image(&u_path, None, api);

                let v_path = rsrc_path(&item["src-v"], &self.aux_dir);
                let (v_key, _) = self.add_or_get_image(&v_path, None, api);
            ...
            "nv12" => {
                ...
                let (uv_key, _) = self.add_or_get_image(&uv_path, None, api);

                YuvData::NV12(y_key, uv_key)
            }
            "interleaved" => {
                let yuv_path = rsrc_path(&item["src"], &self.aux_dir);
                let (yuv_key, _) = self.add_or_get_image(&yuv_path, None, api);
    ...
    fn handle_image(
        ...
        let (image_key, image_dims) =
            self.add_or_get_image(&file, tiling, api);
    ...
    pub fn add_display_list_items_from_yaml(
        ...
        for item in yaml.as_vec().unwrap() {
            ...
            match item_type {
                ...
                "image" => self.handle_image(dl, api, item, &mut info),
                "yuv-image" => self.handle_yuv_image(dl, api, item, &mut info),
                "text" | "glyphs" => unimplemented!(),//self.handle_text(dl, api, item, &mut info),
                "scroll-frame" => self.handle_scroll_frame(dl, api, item),
                "sticky-frame" => self.handle_sticky_frame(dl, api, item),
                "clip" => self.handle_clip(dl, api, item),
                "clip-chain" => self.handle_clip_chain(dl, item),
                "border" => self.handle_border(dl, api, item, &mut info),
                ...
                "stacking-context" => {
                    self.add_stacking_context_from_yaml(dl, api, item, false, &mut info)
                }
                "reference-frame" => self.handle_reference_frame(dl, api, item, &mut info),
    ...
    pub fn handle_scroll_frame(
        ...
        let image_mask = self.to_image_mask(&yaml["image-mask"], api);
        ...
        if !yaml["items"].is_badvalue() {
            ...
            self.add_display_list_items_from_yaml(dl, api, &yaml["items"]);
            dl.pop_clip_id();
        }
    }
    ...
    pub fn handle_clip(&mut self, dl: &mut DisplayListBuilder, api: &RenderApi, yaml: &Yaml) {
    ...
    pub fn get_root_size_from_yaml(&mut self, api: &RenderApi, yaml: &Yaml) -> LayoutSize {
        yaml["bounds"]
            .as_rect()
            .map(|rect| rect.size)
            .unwrap_or(LayoutSize::new(1920f32,1080f32))
    }
    ...
    pub fn push_reference_frame(
        ...
        let default_bounds = LayoutRect::new(LayoutPoint::zero(), LayoutSize::new(1920f32,1080f32));
    ...
    pub fn handle_reference_frame(
        ...
        let reference_frame_id = self.push_reference_frame(dl, api, yaml, info);
