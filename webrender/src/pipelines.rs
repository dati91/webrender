...
impl Device {
    ...
    pub fn create_cache_program(&mut self, vert_src: &[u8], frag_src: &[u8]) -> CacheProgram {
        ...
        let data = cache::Data {
            ...
            ibuf: instances,
            color0: (self.dummy_tex.clone().srv, self.sampler.clone()),
            cache_a8: (self.dummy_tex.clone().srv, self.sampler.clone()),
            cache_rgba8: (self.dummy_tex.clone().srv, self.sampler.clone()),
    ...
    pub fn create_clip_program(&mut self, vert_src: &[u8], frag_src: &[u8]) -> ClipProgram {
        ...
        let data = clip::Data {
            ...
            color0: (self.dummy_tex.clone().srv, self.sampler.clone()),
            color1: (self.dummy_tex.clone().srv, self.sampler.clone()),
            color2: (self.dummy_tex.clone().srv, self.sampler.clone()),
