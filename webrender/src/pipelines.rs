...
type _CachePSO = gfx::PipelineState<R, cache::Meta>;
type _ClipPSO = gfx::PipelineState<R, clip::Meta>;
type _BlurPSO = gfx::PipelineState<R, blur::Meta>;
...
impl BlurInstances {
    ...
    pub fn _update(&mut self, blur_command: &BlurCommand) {
...
impl CacheProgram {
    ...
    pub fn get_pso(&self, blend: &BlendMode) -> &_CachePSO {
        match *blend {
...
pub struct ClipProgram {
    ...
    pub pso: _ClipPSO,
    pub pso_multiply: _ClipPSO,
    pub pso_max: _ClipPSO,
    pub slice: gfx::Slice<R>,
...
impl ClipProgram {
    pub fn new(data: clip::Data<R>,
           psos: (_ClipPSO, _ClipPSO, _ClipPSO),
           slice: gfx::Slice<R>,
    ...
    pub fn get_pso(&self, blend: &BlendMode) -> &_ClipPSO {
        match *blend {
...
impl Device {
    ...
    pub fn _create_cache_psos(&mut self, vert_src: &[u8],frag_src: &[u8]) -> (_CachePSO, _CachePSO) {
        let pso = self.factory.create_pipeline_simple(
            vert_src,
            frag_src,
            cache::new()
    ...
    pub fn _create_clip_psos(&mut self, vert_src: &[u8],frag_src: &[u8]) -> (_ClipPSO, _ClipPSO, _ClipPSO) {
    ...
    pub fn _create_cache_program(&mut self, vert_src: &[u8], frag_src: &[u8]) -> CacheProgram {
        ...
        let psos = self._create_cache_psos(vert_src, frag_src);
        CacheProgram::new(data, psos, self.slice.clone(), upload)
    }

    pub fn _create_blur_program(&mut self, vert_src: &[u8], frag_src: &[u8]) -> BlurProgram {
        let upload = self.factory.create_upload_buffer(MAX_INSTANCE_COUNT).unwrap();
        {
            let mut writer = self.factory.write_mapping(&upload).unwrap();
            for i in 0..MAX_INSTANCE_COUNT {
                writer[i] = BlurInstances::_new();
            }
        }
    ...
    pub fn create_clip_program(&mut self, vert_src: &[u8], frag_src: &[u8]) -> ClipProgram {
        ...
        let psos = self._create_clip_psos(vert_src, frag_src);
        ClipProgram::new(data, psos, self.slice.clone(), upload)
    }

    pub fn _draw_cache(&mut self, program: &mut CacheProgram, proj: &Matrix4D<f32>, instances: &[PrimitiveInstance], blendmode: &BlendMode) {
    ...
    pub fn _draw_blur(&mut self, program: &mut BlurProgram, proj: &Matrix4D<f32>, blur_commands: &[BlurCommand]) {
       program.data.transform = proj.to_row_arrays();

        {
            let mut writer = self.factory.write_mapping(&program.upload).unwrap();
            for (i, blur_command) in blur_commands.iter().enumerate() {
                writer[i]._update(blur_command);
            }
        }
