...
impl<B: hal::Backend> Image<B> {
    ...
    pub fn update(//<T>(
        ...
        //    T: Copy,
    {
        /*let data_stride = match self.image_format {
             ImageFormat::R8 => 1,
             ImageFormat::RG8 => 2,
             ImageFormat::BGRA8 => 4,
             _ => unimplemented!("TODO image format missing"),
         };*/
        //let buffer_width = (image_data.len() /* * data_stride as usize*/) as u64;
        //self.image_upload_buffer
        //    .update(device, 0, buffer_width, image_data);

        let (image_width, image_height, _, _) = self.image_kind.get_dimensions();
        let pos = rect.origin;
        let size = rect.size;
        //let data_stride = self.image_upload_buffer.data_stride;
        //let buffer_offset = pos.y as u64 * image_width as u64 + pos.x as u64;
        //self.image_upload_buffer
        //    .update(device, buffer_offset, (size.width * size.height * self.imadata_stride) as _, image_data);
        ...
        cmd_buffer.copy_buffer_to_image(
            ...
            &[
                hal::command::BufferImageCopy {
                    ...
                    //buffer_width: image_data.len() as u32, // / (image_width / 4) as u32,
                    buffer_width: size.width,// * self.image_upload_buffer.buffer.data_stride as u32,
                    //buffer_width: image_width as _,
                    //buffer_height: image_data.len() as u32 / (image_width / 4) as u32,
                    buffer_height: size.height,// * self.image_upload_buffer.buffer.data_stride as u32,
                    //buffer_height: image_height as _,
                    //buffer_height: 0,
            ...
            device.device.update_descriptor_sets::<Range<_>>(&[
                hal::pso::DescriptorSetWrite {
                    ...
                    write: hal::pso::DescriptorWrite::SampledImage(vec![
                        (
                            &device.images.get(&device.bound_textures[0]).unwrap().image_srv,
...
impl<B: hal::Backend> Device<B, hal::Graphics> {
    ...
    {
        //println!("upload_texture.rect={:?}", rect);
        //println!("upload_texture.layer_index={:?}", layer_index);
        //println!("upload_texture.stride={:?}", stride);
        //println!("upload_texture.data.len={:?}", data.len());
        let data_stride = match texture.format {
            ImageFormat::R8 => 1,
            ImageFormat::RG8 => 2,
            ImageFormat::BGRA8 => 4,
            _ => unimplemented!("TODO image format missing"),
         };
        let size = rect.size.width * rect.size.height * data_stride;
        let mut new_data = vec![0u8; size as usize];
        let data= if stride.is_some() {
            //println!("TODO image upload with stride");
            let row_length = (stride.unwrap()) as usize;

            for j in 0..rect.size.height as usize {
                for i in 0..rect.size.width as usize {
                    let offset = i*data_stride as usize + j*data_stride as usize *rect.size.width as usize;
                    let src = &data[j * row_length + i * data_stride as usize ..];
        ...
        //println!("rect={:?}", rect);
        assert_eq!(data.len() as u32, rect.size.width * rect.size.height * data_stride);
        self.upload_queue
            .push(
                self.images
                    .get_mut(&texture.id)
